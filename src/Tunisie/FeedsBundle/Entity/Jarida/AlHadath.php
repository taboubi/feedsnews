...
class AlHadath
{
    ...
    public function __construct($em) {
        $this->url = 'http://ar.webmanagercenter.com/%D8%A7%D9%84%D8%AD%D8%AF%D8%AB/';
        $this->em = $em;
    ...
    public function getPages($html){
        ...
        preg_match_all('/<a href=\'(.*?)\' class=Titre_liste target=\'_self\'>/', $html, $matches);  

        foreach($matches[1] as $dh){
            if($this->proceed){

 
                   // $a = 'http://www.aljarida.com.tn'.utf8_decode($link->getAttribute('href'));
                    $html = $this->execCurl($dh) ;
                    preg_match('/<meta property="og:image" content="(.*?)">/', $html, $matches);
                    $image = $matches[1] ;

                    preg_match('/<meta property="og:title" content="(.*?)">/', $html, $matches);
                    $title = $matches[1];
                    preg_match_all('/<div class="contenu">(.*?)<\/div>/s', $html, $matches); 
                    $string = preg_replace('/<a(.*?)<\/a>/', '', $matches[0][0]);
                    $description = strip_tags($string,'<p><br>');
